<?php
	...
	class ApiController extends CoreControllerLite {
		 ...
		 * API to do something unauthorized. Alternately the
		 * API can be set to run on a different subdomain that
		 * can't read the regular site's cookies.
		 *
		 * @access protected
		...
		protected function authenticate() {
			if (!AppConfig::get('ApiInternal', false)) {
				if (AppRegistry::get('Url')->getVariable('sid') == session_id()) {
					return AppRegistry::get('UserLogin')->isLoggedIn();
				} else if (!empty($_SERVER['PHP_AUTH_USER']) && !empty($_SERVER['PHP_AUTH_PW'])) {
					AppConfig::set('NoLoginCookie', true);
					return AppRegistry::get('UserLogin')->handleFormLogin($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW']);
				}
			}
		}
		...
		protected function encode($blnSuccess, array $arrResult = array()) {
			if ($this->blnDebug) {
				$arrResult['timer'] = $_GLOBALS['objTimer']->getTime();
			}
