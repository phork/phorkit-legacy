...
}else{b.show(d)}});this.$parent.data("items").hover(function(d){b.show(d)})};this.append=function(b){if(!this.$parent.data("trigger")){this.init()}var c=this.$parent.data("items");
$(b).each(function(d,e){if(typeof e=="object"){c.append($appended=$("<li></li>").append($("<a>").append(e.title).attr({href:e.href})));if(e.spacer){$appended.addClass("spacer")
}if(e.id){$appended.attr("id",e.id)}}});return this};this.hover=function(f,d,b){if(!(this.hovered[d]=b)){var c=this;setTimeout(function(){c.hide()},100)
}};this.show=function(b){this.$parent.addClass("hover");this.$parent.data("items").css({position:"absolute",left:this.$parent.offset().left+this.offset.left,top:this.$parent.offset().top+this.$parent.height()+this.offset.top}).stop(true,true).slideDown("fast").show()
