...
}else{$(this).html("Uh oh. Something went wrong.");d.$overlay.addClass("error")}if(typeof d.config.initialized=="function"){d.config.initialized(d,[h=="success"])
}d.resize()});break;default:if(typeof this.config.initialized=="function"){this.config.initialized(this,true)}this.$overlay.addClass("initialized");break
}if(this.config.buttons){this.$overlay.append(this.$buttons=$("<div></div>").addClass("overlay-buttons"));for(var f in this.config.buttons){(function(){var h=d.config.buttons[f];
d.$buttons.append($("<button></button>").html(h.value).bind("click",function(){if(typeof h.action=="function"){h.action(d)}if(h.close){setTimeout(function(){d.hide()
},100)}}).addClass(h.type).data(h.data?h.data:{}))})()}}return this};a.prototype.resize=function(f){var d=this.$content.width(),h=this.$content.height(),e=this.$content.attr("scrollWidth"),g=this.$content.attr("scrollHeight"),j=e-d,i=g-h;
...
this.$content.height(h+i)}return this.position()};a.prototype.position=function(){var e=$(window).width(),i=$(window).height(),d=this.$overlay.width(),h=this.$overlay.height(),g=Math.round(e*0.8),f=Math.round(i*0.8);
